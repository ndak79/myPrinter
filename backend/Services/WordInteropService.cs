...
public class WordInteropService : IWordInteropService
{
    ...
    /// <summary>
    /// T?o 1 trang "blank" nhung có content siêu nh? d? tránh b? viewer/driver skip.
                                                   ...
                                                   bool isLandscape)
    {
        ...
        // L?y kích thu?c t? template, r?i ch?nh orientation n?u c?n.
        ...
        {
            // V? 1 hình ch? nh?t nh? màu xám ? G?N GI?A TRANG
            // ? ch?c ch?n n?m trong vùng in, có pixel khác tr?ng.
            double rectSize = 3; // kích thu?c r?t nh?, khó th?y
    ...
    public void PrintPdf(string pdfPath, string printerName, string? pageRange = null, string? duplexSide = null)
    {
        ...
        try
        {
            ...
            if (!printSuccess)
            {
                Console.WriteLine($"[PrintPdf] Shell print failed, trying alternate shell fallback...");
                TryPowerShellPrint(fileToPrint, printerName);
                // NOTE: legacy fallback does NOT support duplexSide — acceptable degraded behavior
            }
    ...
    private bool PrintWithSumatra(string sumatraPath, string pdfPath, string printerName, string? duplexSide = null)
    {
        try
        {
            var psi = BuildSumatraPrintStartInfo(sumatraPath, pdfPath, printerName, duplexSide);
            Console.WriteLine($"[PrintWithSumatra] Args: {string.Join(" ", psi.ArgumentList)}");
    ...
    internal static System.Diagnostics.ProcessStartInfo BuildSumatraPrintStartInfo(
        string sumatraPath,
        string pdfPath,
        string printerName,
        string? duplexSide = null)
    {
        ...
        var printSetting = duplexSide switch
        {
        ...
        if (printSetting != null)
        {
            ...
            startInfo.ArgumentList.Add(printSetting);
        }
    ...
    private bool PrintBySwappingDefaultPrinter(string filePath, string printerName)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var startInfo = BuildShellPrintStartInfo(filePath);
    ...
    internal static System.Diagnostics.ProcessStartInfo BuildShellPrintStartInfo(string filePath)
    {
    ...
    {
        Console.WriteLine("[TryPowerShellPrint] Using shell print fallback without PowerShell.");
        _ = PrintBySwappingDefaultPrinter(filePath, printerName);
    }
    ...
    private void CleanupWordObjects(Word.Document? doc, Word.Application? app)
    {
        ...
        /// <summary>
    /// X? lý mixed-orientation + single-sided:
    /// - Nhóm các trang liên ti?p cùng orientation.
    /// - V?i trang single-sided: chèn thêm 1 trang blank cùng orientation ngay sau nó.
    /// - Sau m?i group: n?u t?ng s? trang trong group là l? ? chèn 1 trang blank d? thành ch?n.
    /// K?t qu?:
    /// - File PDF m?i v?i s? trang ch?n, các group orientation không b? "xé t?".
    /// - Danh sách ManualDuplexPageInfo d? build ManualDuplexPlan.
    /// </summary>
                                          ...
                                          HashSet<int>? insertedBlankIndices = null)
    {
        ...
        try
        {
            ...
            // 1) Tính orientation cho t?ng trang g?c
            ...
            for (int i = 0; i < pageCount; i++)
            {
                ...
                // N?u Rotate 90/270 thì width/height d?i vai trò
                int rotate = p.Rotate;
                if (rotate == 90 || rotate == 270)
                {
                    (w, h) = (h, w);
            ...
            // 2) Duy?t theo group consecutive cùng orientation
            int current = 1;
            while (current <= pageCount)
            {
                bool groupOrientation = isLandscapeByPage[current];
                int groupStartLogicalIndex = pageInfos.Count; // dánh d?u d? tính size group sau
                ...
                // 2a) X? lý t?ng trang trong group
                foreach (var pageNum in groupPageNumbers)
                {
                    ...
                    {
                        // ProcessedIndex s? set sau, khi dã có full list
                    ...
                    if (isSingleSided)
                    {
                        // Thêm 1 blank ngay sau d? d?m b?o trang này có m?t sau tr?ng
                        pageInfos.Add(new ManualDuplexPageInfo
                        {
                ...
                // 2b) Padding group thành ch?n (n?u c?n)
                ...
                {
                    // Thêm 1 blank cùng orientation d? group k?t thúc dúng cu?i t?
                    pageInfos.Add(new ManualDuplexPageInfo
                    {
                        OriginalPageNumber = -1,
                        IsBlank = true,
            ...
            // 3) D?a trên pageInfos ? t?o file PDF m?i
    ...
    /// <summary>
    /// Overload cu: gi? cho các ch? g?i hi?n t?i không b? v?.
    /// N?u không c?n plan chi ti?t thì dùng hàm này.
    /// </summary>
    ...
    public string CreateSmartDuplexPdf(string sourcePath, int[] pageNumbers, FlipDirection? flipDirectionOverride = null)
    {
        ...
        try
        {
            ...
            // Th? t? dã du?c ManualDuplexPlan.Build() x? lý dúng (Reverse cho faceDownStack).
            // KHÔNG OrderByDescending l?i — ch? l?c out-of-range d? tránh crash.
            ...
            foreach (var pageNum in orderedPages)
            {
                ...
                // Tính orientation th?c t? (k? c? Rotate 90/270)
                ...
                {
                    // Không xoay: import tr?c ti?p t? sourceDoc
            ...
            targetDoc.Save(outputPath);                 // ? Save dúng 1 l?n
